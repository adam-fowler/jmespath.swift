...
internal class JSONMap {
    ...
    let mapBuffer: [Int]
    var data: (buffer: BufferView, allocation: UnsafePointer<UInt8>?)

    init(mapBuffer: [Int], dataBuffer: BufferView) {
        self.mapBuffer = mapBuffer
        self.data = (buffer: dataBuffer, allocation: nil)
    }

    func copyInBuffer() {

        guard self.data.allocation == nil else {
            return
        }
        ...
        let (p, c) = self.data.buffer.withUnsafePointer { pointer, count -> (UnsafePointer, Int) in
            let raw = UnsafeMutablePointer<UInt8>.allocate(capacity: count + 1)
            raw.initialize(from: pointer, count: count)
            raw[count] = 0  //.storeBytes(of: UInt8.zero, toByteOffset: capacity, as: UInt8.self)
            return (.init(raw), count + 1)
        }

        self.data = (buffer: .init(unsafeBaseAddress: p, count: c), allocation: p)
    }

    @inline(__always)
    func withBuffer<T>(
        for region: Region,
        perform closure: @Sendable (_ jsonBytes: BufferView, _ fullSource: BufferView) throws -> T
    ) rethrows -> T {
        try closure(self.data.buffer[region], self.data.buffer)
    }

    deinit {
        if let allocatedPointer = self.data.allocation {
            precondition(self.data.buffer.startIndex == BufferView.Index(rawValue: allocatedPointer))
...
internal struct JSONScanner {
    ...
    init(bytes: BufferView, options: Options) {
        ...
        while let next = reader.peek(offset: whitespace) {
            ...
            default:
                throw JSONError.unexpectedCharacter(
                    ...
                    ascii: next,
                    location: reader.sourceLocation(atOffset: whitespace)
                )
            }
        }
    ...
    mutating func scanString() throws {
        ...
        // skip initial quote
        return partialMap.record(
            tagType: isSimple ? .simpleString : .string,
            count: reader.distance(from: start, to: end),
            dataOffset: reader.byteOffset(at: start),
            with: reader
        )
    }
    ...
    mutating func scanNumber() throws {
        ...
        let end = reader.readIndex
        return partialMap.record(
            tagType: containsExponent ? .numberContainingExponent : .number,
            count: reader.distance(from: start, to: end),
            dataOffset: reader.byteOffset(at: start),
            with: reader
        )
...
extension JSONScanner {
    ...
    struct DocumentReader {
        let bytes: BufferView
        private(set) var readIndex: BufferView.Index
        private let endIndex: BufferView.Index
        ...
        @inline(__always)
        func byteOffset(at index: BufferView.Index) -> Int {
            bytes.distance(from: bytes.startIndex, to: index)
        }

        init(bytes: BufferView) {
            self.bytes = bytes
        ...
        @inline(__always)
        func distance(from start: BufferView.Index, to end: BufferView.Index) -> Int {
        ...
        mutating func skipUTF8StringTillNextUnescapedQuote(isSimple: inout Bool) throws -> BufferView.Index {
            // Skip the open quote.
...
extension JSONScanner {
    ...
    static func stringValue(
        from jsonBytes: BufferView,
        fullSource: BufferView
    ) throws -> String {
    ...
    static func _slowpath_stringValue(
        from jsonBytes: BufferView,
        appendingTo output: inout String,
        fullSource: BufferView
    ) throws {
    ...
    internal static func parseUnicodeSequence(
        ...
        if UTF16.isLeadSurrogate(bitPattern) {
            ...
            guard UTF16.isTrailSurrogate(trailingSurrogateBitPattern) else {
                throw JSONError.expectedLowSurrogateUTF8SequenceAfterHighSurrogate(
                    location: .sourceLocation(at: trailingBytes.startIndex, fullSource: fullSource)
                )
            }
            ...
            guard let unicode = Unicode.Scalar(bitPattern) else {
                throw JSONError.couldNotCreateUnicodeScalarFromUInt32(
                    location: .sourceLocation(at: jsonBytes.startIndex, fullSource: fullSource),
                    unicodeScalarValue: UInt32(bitPattern)
                )
            }
            string.unicodeScalars.append(unicode)
            return index1
    ...
    internal static func parseUnicodeHexSequence(
        ...
        fullSource: BufferView,
        allowNulls: Bool = true
    ) throws -> (scalar: UInt16, nextIndex: BufferView.Index) {
    ...
    static func validateLeadingZero(in jsonBytes: BufferView, fullSource: BufferView) throws {
    ...
    static func prevalidateJSONNumber(
        ...
        let firstDigitIndex: BufferView.Index
        switch jsonBytes[uncheckedOffset: 0] {
        case UInt8(ascii: "0"):
        ...
        case UInt8(ascii: "-"):
            ...
            case let byte:  // default
                ...
                throw JSONError.unexpectedCharacter(
                    ...
                    ascii: byte,
                    location: .sourceLocation(at: jsonBytes.index(after: jsonBytes.startIndex), fullSource: fullSource)
                )
            }
            ...
            exponentLoop: while index < jsonBytes.endIndex {
                ...
                case UInt8(ascii: "e"), UInt8(ascii: "E"):
                    ...
                    guard case _asciiNumbers = jsonBytes[unchecked: previous] else {
                        throw JSONError.unexpectedCharacter(
                            context: "in number",
                            ascii: jsonBytes[index],
                            location: .sourceLocation(at: index, fullSource: fullSource)
                        )
                    }
        ...
        case let lastByte:  // default
            throw JSONError.unexpectedCharacter(
                context: "at end of number",
                ascii: lastByte,
                location: .sourceLocation(at: lastIndex, fullSource: fullSource)
            )
        }
    ...
    static func validateNumber(from jsonBytes: BufferView, fullSource: BufferView) -> JSONError {
...
protocol PrevalidatedJSONNumberBufferConvertible {
    init?(prevalidatedBuffer buffer: BufferView)
}
...
extension Double: PrevalidatedJSONNumberBufferConvertible {
    init?(prevalidatedBuffer buffer: BufferView) {
        let decodedValue = buffer.withUnsafePointer { nptr, count -> Double? in
            ...
            let decodedValue = strtod(nptr, &endPtr)
...
extension Float: PrevalidatedJSONNumberBufferConvertible {
    init?(prevalidatedBuffer buffer: BufferView) {
        let decodedValue = buffer.withUnsafePointer { nptr, count -> Float? in
            var endPtr: UnsafeMutablePointer<CChar>? = nil
            let decodedValue = strtof(nptr, &endPtr)
            if let endPtr, nptr.advanced(by: count) == endPtr {
                return decodedValue
            } else {
                return nil
...
internal func _parseInteger<Result: FixedWidthInteger>(_ codeUnits: BufferView) -> Result? {
...
enum JSONError: Swift.Error, Equatable {
    ...
    var sourceLocation: SourceLocation? {
        ...
        case let .invalidHexDigitSequence(_, location), let .invalidEscapedNullValue(location), let .invalidSpecialValue(_, location):
            return location
        case let .unexpectedEscapedCharacter(_, location), let .unescapedControlCharacterInString(_, location),
            let .expectedLowSurrogateUTF8SequenceAfterHighSurrogate(location):
            return location
